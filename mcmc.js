...
window.AtlasMCMC = (() => {
  ...
  async function readJSON(path) {
    const response = await fetch(`${path}?v=mcmc-20260914-1`);
  ...
  function render(archive) {
    ...
    status.textContent = t(archive.status === "ready" ? "mcmcLoaded" : archive.status === "error" ? "mcmcFailed" : "mcmcLoading");
    if (archive.status !== "ready") return;
    const {lf, joint, categories} = archive.catalog;
    const model = lf.models[archive.selectedLF];
    const imagePath = `${categories.lf_only.directory}/${model.file}`;
    ...
    node("mcmc-lf-caption").textContent = t("mcmcLFCaption", {kp: model.KP_h_Mpc, ms: model.MS, job: lf.chain_source});
    ...
    node("mcmc-lf-models").querySelectorAll("button").forEach((button, index) => {
      const entry = lf.models[index];
      button.setAttribute("aria-pressed", String(index === archive.selectedLF));
      button.setAttribute("aria-label", t("mcmcModelOption", {kp: entry.KP_h_Mpc, ms: entry.MS}));
    });
  }
  ...
  async function initialize(archive) {
    ...
    try {
      ...
      const controls = node("mcmc-lf-models");
      controls.replaceChildren();
      lf.models.forEach((model, index) => {
        const button = document.createElement("button");
        button.type = "button";
        button.textContent = `kₚ ${model.KP_h_Mpc} / mₛ ${model.MS}`;
        button.addEventListener("click", () => { archive.selectedLF = index; render(archive); });
