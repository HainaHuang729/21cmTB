...
window.AtlasI18n = (() => {
  const messages = {
    ...
    mcmcLFCaution: ["Both ensembles use the saved length listed above. Passing the existing diagnostic gate is not proof of global mixing or numerical-resolution convergence. These figures are exploratory, not final parameter constraints.","两组独立采样均使用上方列出的保存长度。通过现有诊断门槛不代表已经证明全局混合充分或数值分辨率收敛。这些图为探索性结果，不是最终参数约束。"],
    mcmcLFCoordinates: ["Native coordinates: log₁₀ f★,10, t★, α★, log₁₀(Mturn/M☉). Conversion only: log₁₀ f★,10 = η★ + log₁₀ t★. Blue / ensemble 0 · Orange / ensemble 1 · Solid / 68% · Dashed / 95% binned sample mass; not validated credible regions.","原始坐标：log₁₀ f★,10、t★、α★、log₁₀(Mturn/M☉)。仅作坐标变换：log₁₀ f★,10 = η★ + log₁₀ t★。蓝色 / ensemble 0 · 橙色 / ensemble 1 · 实线 / 68% · 虚线 / 95% 分箱样本质量；并非已验证的可信区域。"],
    ...
    mcmcLFCaption: ["Fixed kₚ = {kp} h Mpc⁻¹ / mₛ = {ms} · Source / {job} · {date} · Exploratory snapshot","固定 kₚ = {kp} h Mpc⁻¹ / mₛ = {ms} · 来源 / {job} · {date} · 探索性快照"],
