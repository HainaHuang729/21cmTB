...
window.AtlasI18n = (() => {
  const messages = {
    ...
    mcmcIndependent: ["LF-only corners follow kₚ and mₛ in the bottom parameter dock. Other sliders do not condition these four-dimensional sampling distributions. Switching models only loads stored figures; no chain is recomputed or reweighted. The free-mₛ joint archive remains a separate fixed snapshot.","LF-only corner 随底部参数栏的 kₚ、mₛ 切换。其余滑块不会对四维采样分布施加条件约束；切换仅加载预存图像，不重新计算或重加权链。自由 mₛ 的联合约束归档仍单独显示固定快照。"],
    ...
    mcmcLFUnavailableDetail: ["No stored LF-only corner for kₚ = {kp} h Mpc⁻¹, mₛ = {ms}. The archive contains kₚ = 1, 3, 10, 20, 30 and mₛ = 1.5, 2, 2.5, 3.25, 4; kₚ = 0 (PL) is not included. No substitute model is shown.", "没有 kₚ = {kp} h Mpc⁻¹、mₛ = {ms} 对应的 LF-only corner。现有归档包含 kₚ = 1、3、10、20、30 与 mₛ = 1.5、2、2.5、3.25、4，不包含 kₚ = 0（PL）。不会显示其他参数模型作为替代。"],
