...
LF-only（三组模型可切换）和七参数、自由 ms 的 LF + τ + ξ 两类。
归档独立于模拟参数面板，不进行重加权或重新采样。LF-only 尚未确认收敛；
联合快照仍未收敛。样本记录数不等于有效独立样本数，不能将图作为最终约束。
详情与来源见 [revision 21 notes](UPDATES-v21.md)。
