...
分辨率、宇宙学和随机种子的标准 PL 参考；点击底部五个正方形字段切片会
更新中部的大型 BPL/PL 对照主图，非亮温场在两种模型之间使用共同色标。
参考模型使用
`POWER_SPECTRUM=0, POWER_INDEX=0.968`，不再使用“上一选择”曲线。

网页采用紧凑的学术数据 Dashboard：顶部横排 10 个参数；其下是全宽
brightness-temperature lightcone；中部为大型二维场主图和两个统计辅助图；
底部为五个等宽正方形切片。第二个辅助图可在再电离历史与 UV LF 之间切换。
