...
## PL / BT 三维对比 / 2026-09-22

入口：[PL × BT Viewer](https://hainahuang729.github.io/21cmTB/viewer/compare.html)。
两组全新 256³、250 cMpc 模拟，各 85 个真实快照（z=35.216929→6）。
公网展示 64³ 体积平均档，支持同步相机、时间轴、裁切、X/Y/Z 切片和中英文。
128³/256³ 暂不托管；不与旧的独立 64³、100 cMpc Viewer 数据混合。

展示数据保存在 [`viewer-data64/pl-bt-v1`](viewer-data64/pl-bt-v1)，由网页按需读取，
不打包进 Pages。只上传派生的 xH 和密度；原始 HDF5 未上传。
BT 亮温的原始缓存存在极端有限值，尚待物理核查，因此亮温对比明确禁用。
同一随机种子不等于已验证相位匹配；本页面不作逐体素差分解释。

## 当前数据更新 / 2026-09-18

当前展示的 MCMC 数据统一采用 `MUV > −20`：25 组 BPL LF-only
使用新链 `2135547_0–24`（每组 15000 步 × 32 walkers × 2 ensembles），
共享 PL 使用延长后的 30000 步链。两者使用相同的 34 个 HST + JWST
观测点与原先验。LF 曲线按各链最大 LF 似然样本重新计算，并核对保存似然。
Park / PL 合并图也更新为同一份 PL；Park 仍保留其 27 个 HST 点及独立先验。

联合分支替换为 `2135863` 的 `MUV > −20` 完成功能测试：固定
`kp=10, ms=2.5`、八参数、256³、3 步 × 16 walkers。仅展示原始散点，
不作为后验或可信区间；正在续跑的数据未纳入。旧的自由 ms 七参数文件
作为历史证据保留，不再从当前页面加载。本次没有重跑模拟或 MCMC。
下方旧版本说明仅描述历史设计，以本节和当前 JSON 数据清单为准。

Park 与 MUV > −20 的 PL 对照现统一到
[合并 corner 与最佳拟合 LF](park-pl-comparison.html)。主页只保留这一个入口；
两个旧的独立展示页跳转到合并页。最新合并图、数值、收敛标记及原始归档数据
均保留，不重跑 MCMC。主图谱的 BPL/PL 参数联动与联合约束不受影响。
...
和气体动温 `Tk_box` 场。
UV luminosity function（LF）同时展示 `z = 6, 7, 8, 10` 四个面板，使用统一坐标范围。
...
BPL / matched-PL 切片。切片下方是 MCMC 归档，分为四参数、固定 kp/ms 的
LF-only（完整 5×5 网格、25 组模型可切换）和七参数、自由 ms 的 LF + τ + ξ 两类。
LF-only corner 由底部参数栏的 `KP`、`MS` 联动切换，不再单独选择模型；
`KP=0` 没有对应的 LF-only 链时明确显示无数据，不以其他参数替代。
恒星形成参数滑块不会对四维分布施加条件或重加权；联合约束因自由采样 `MS`
仍显示独立快照，不随底部参数变化。LF-only 按模型显示保存
步数和原报告的诊断门槛结果，门槛通过也不等于最终约束；联合快照仍未收敛。
样本记录数不等于有效独立样本数。两类 corner 使用统一的蓝/橙 ensemble
配色、阶梯直方图和实线/虚线轮廓，坐标数字只在外侧显示，避免相邻面板重叠。
最新绘图说明见 [revision 23 notes](UPDATES-v23.md)，采样来源仍见
[revision 22 notes](UPDATES-v22.md)。
...
页面严格按照这个设计工作。BPL 模式移动一个天体物理参数时，其他九个参数回到
基准值；PL 模式保持 `KP=0` 和当前 `MS`，只重置其余天体物理参数。
`KP` 与 `MS` 可以联合移动。每一帧都对应一个真实 21cmFAST
lightcone，不进行跨模型场插值。
...
的中心列采样视图，并明确标注；不将 BPL 光锥充作 PL。PL 模式调节天体物理
参数会加载对应的 PL 参考，保持 `KP=0`；点击重置恢复基准 `KP=1, MS=1.5`。
...
演化。LF 四个红移同时展示，不随切片滑块变化；二者都不进行
在线计算。
