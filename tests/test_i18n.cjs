...
test("all 25 corners follow dock KP/MS with provenance, no substitute for PL, and bilingual copy", async () => {
  ...
  assert.equal(h.get("#mcmc-lf-open").hidden, false);
  assert.equal(h.get("#mcmc-lf-empty").hidden, true);
  const plPath = h.get("#mcmc-lf-image").getAttribute("src");
  assert.match(plPath, /lf_corner_pl.png/);
  assert.equal(h.get("#mcmc-lf-open").getAttribute("href"), plPath);
  assert.ok(fs.existsSync(path.join(staticRoot, plPath.split("?")[0])));
  assert.match(h.get("#mcmc-lf-fixed").textContent, /PL/);
  assert.equal(h.get("#mcmc-lf-rows").textContent, archive.lf.pl_only.sample_count.toLocaleString("zh-CN"));
  moveDock(h, "MS", 2);
  assert.equal(h.run("state.mcmc.selectedLF"), -1);
  assert.equal(h.get("#mcmc-lf-image").getAttribute("src"), plPath);
  h.run('setLanguage("en")');
  assert.match(h.get("#mcmc-lf-coordinates").textContent, /Blue dashed: PL/);
  assert.equal(h.get("#mcmc-lf-image").getAttribute("src"), plPath);
